...
import sys
sys.path.append("../../")
from lib.multiheaded_attention_dynamic_sparse import multiheaded_attention
...
class Transformer:
    ...
    def __init__(self, hyperparameters, query, value, dim,
                 duty_cycles, duty_index, new_duty_cycles, beta, current_depth, layers,
    ...
    def single_unit(self, Q, V, t):
        ...
        with tf.variable_scope(self.name+"_"+str(t), reuse=tf.AUTO_REUSE, dtype=tf.float32):
            ...
            maskW1 = tf.get_variable("maskW1", [self.D, self.fc_dim], dtype=tf.float32,
                                     initializer=tf.ones_initializer())

            mask = tf.nn.sigmoid(self.inf*maskW1)
            W1 = mask*W1
            ...
            maskW2 = tf.get_variable("maskW2", [self.fc_dim, self.D], dtype=tf.float32,
                                     initializer=tf.ones_initializer())

            mask = tf.nn.sigmoid(self.inf*maskW2)
            W2 = mask*W2
            self.sparse_reg += tf.reduce_sum(tf.abs(mask))
        ...
        sublayer2 = k_winner(tf.matmul(sublayer2, W1)+B1)
