    ...
    def multiheaded_attention(self):
        ...
        with tf.variable_scope(self.name+"_"+str(t), reuse=tf.AUTO_REUSE, dtype=tf.float32):
            ...
            Wq = tf.get_variable("Wq",
                                 dtype=tf.float32, initializer=tf.constant(sparse_init((self.heads, self.D, self.d), l)))
            ...
            if self.adaptive_span:
                ...
                Vz = tf.get_variable("Vz",
                                     dtype=tf.float32, initializer=tf.constant(sparse_init((self.heads, self.d, 1), l)))
            ...
            Wo = tf.get_variable("Wo",
                                 dtype=tf.float32, initializer=tf.constant(sparse_init((self.heads*self.d, self.out_dim), l)))
