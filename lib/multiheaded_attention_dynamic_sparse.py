...
class multiheaded_attention:
    ...
    def __init__(self, hyperparameters, query, value, dim, beta,
    ...
    def multiheaded_attention(self):
        ...
        with tf.variable_scope(self.name+"_"+str(t), reuse=tf.AUTO_REUSE, dtype=tf.float32):
            ...
            maskWq = tf.get_variable("maskWq", [self.heads, self.D,  self.d],
                                     dtype=tf.float32, initializer=tf.ones_initializer())

            maskWk = tf.get_variable("maskWk", [self.heads, self.D, self.d],
                                     dtype=tf.float32, initializer=tf.ones_initializer())

            maskWv = tf.get_variable("maskWv", [self.heads, self.D, self.d],
                                     dtype=tf.float32, initializer=tf.ones_initializer())

            mask = tf.nn.sigmoid(self.inf*maskWq)
            Wq = mask*Wq
            self.sparse_reg += tf.reduce_sum(tf.abs(mask))

            mask = tf.nn.sigmoid(self.inf*maskWk)
            Wk = mask*Wk
            self.sparse_reg += tf.reduce_sum(tf.abs(mask))

            mask = tf.nn.sigmoid(self.inf*maskWv)
            Wv = mask*Wv
            self.sparse_reg += tf.reduce_sum(tf.abs(mask))
            ...
            if self.adaptive_span:
                ...
                Vz = tf.get_variable("Vz", [self.heads, self.d, 1],
                                     dtype=tf.float32, initializer=init)
                maskVz = tf.get_variable("maskVz", [self.heads, self.d, 1],
                                         dtype=tf.float32, initializer=tf.ones_initializer())

                mask = tf.nn.sigmoid(self.inf*maskVz)
                Vz = mask*Vz
                self.sparse_reg += tf.reduce_sum(tf.abs(mask))
            ...
            maskWo = tf.get_variable("maskWo", [self.heads*self.d, self.out_dim],
                                     dtype=tf.float32, initializer=tf.ones_initializer())

            mask = tf.nn.sigmoid(self.inf*maskWo)
            Wo = mask*Wo
            self.sparse_reg += tf.reduce_sum(tf.abs(mask))
        ...
        with tf.variable_scope(self.name, reuse=tf.AUTO_REUSE, dtype=tf.float32):
            ...
            maskWrk = tf.get_variable("maskWrk", [self.heads, self.D, self.d],
                                      dtype=tf.float32, initializer=tf.ones_initializer())

            mask = tf.nn.sigmoid(self.inf*maskWrk)
            Wrk = mask*Wrk
