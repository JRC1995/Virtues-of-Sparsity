...
import sys
sys.path.append("../../")
from lib.Transformer_sparse import Transformer
...
class Encoder:
    ...
    def __init__(self, config_key, text, labels, duty_cycles, dim,
    ...
    def initiate_graph(self):
        ...
        with tf.variable_scope("embedding"):
            ...
            Wc = tf.get_variable("Wc",
                                 dtype=tf.float32,
                                 trainable=True,
                                 initializer=tf.constant(sparse_init((self.D, self.dim))))
        ...
        attention = Transformer(hyperparameters=self.hyperparameters,
                                ...
                                new_duty_cycles=self.new_duty_cycles,
                                duty_index=0,
                                current_depth=1,
                                layers=self.T,
        ...
        self.Z = attention.Z
        self.new_duty_cycles = attention.new_duty_cycles
        self.duty_index = attention.duty_index

        top_layer = layers[-1]

        with tf.variable_scope("softmax"):

            self.W_score1 = tf.get_variable("W_score1",
                                            dtype=tf.float32,
                                            trainable=True,
                                            initializer=tf.constant(sparse_init((self.dim, 300))))

            self.B_score1 = tf.get_variable("Bias_score1",
                                            shape=[300],
                                            trainable=True, initializer=tf.zeros_initializer())

            self.W_score2 = tf.get_variable("W_score2",
                                            dtype=tf.float32,
                                            trainable=True,
                                            initializer=tf.constant(sparse_init((300, self.tags_size))))
        ...
        self.new_duty_cycles = tf.concat(self.new_duty_cycles, axis=-1)
        self.new_duty_cycles = tf.reshape(self.new_duty_cycles, [-1])
