...
import sys
sys.path.append("../../")
from lib.Transformer_dynamic_sparse import Transformer
...
class Encoder:
    ...
    def __init__(self, config_key, text, labels, dim,
                 true_seq_len, beta, train, learning_rate,
        ...
        self.sparse_lambda = hyperparameters['sparse_lambda']
    ...
    def initiate_graph(self):
        ...
        with tf.variable_scope("embedding"):
            ...
            maskWc = tf.get_variable("maskWc", shape=[self.D, self.dim],
                                     dtype=tf.float32,
                                     trainable=True,
                                     initializer=tf.ones_initializer())

            mask = tf.nn.sigmoid(self.inf*maskWc)
            Wc = mask*Wc
            self.sparse_reg += tf.reduce_sum(tf.abs(mask))
        ...
        self.Z = attention.Z
        self.sparse_reg += attention.sparse_reg

        top_layer = layers[-1]

        with tf.variable_scope("softmax"):

            W_score1 = tf.get_variable("W_score1", shape=[self.dim, 300],
                                       dtype=tf.float32,
                                       trainable=True,
                                       initializer=tf.glorot_uniform_initializer())

            maskW_score1 = tf.get_variable("maskW_score1", shape=[self.dim, 300],
                                           dtype=tf.float32,
                                           trainable=True,
                                           initializer=tf.ones_initializer())

            mask = tf.nn.sigmoid(self.inf*maskW_score1)
            W_score1 = mask*W_score1
            self.sparse_reg += tf.reduce_sum(tf.abs(mask))

            B_score1 = tf.get_variable("Bias_score1", dtype=tf.float32,
                                       shape=[300],
                                       trainable=True, initializer=tf.zeros_initializer())

            W_score2 = tf.get_variable("W_score2", shape=[300, self.tags_size],
                                       dtype=tf.float32,
                                       trainable=True,
                                       initializer=tf.glorot_uniform_initializer())

            maskW_score2 = tf.get_variable("maskW_score2", shape=[300, self.tags_size],
                                           dtype=tf.float32,
                                           trainable=True,
                                           initializer=tf.ones_initializer())

            B_score2 = tf.get_variable("Bias_score2", dtype=tf.float32,
                                       shape=[self.tags_size],
                                       trainable=True, initializer=tf.zeros_initializer())

            mask = tf.nn.sigmoid(self.inf*maskW_score2)
            W_score2 = mask*W_score2
            self.sparse_reg += tf.reduce_sum(tf.abs(mask))

            top_layer = tf.reshape(top_layer, [-1, self.dim])

            score1 = gelu(tf.matmul(top_layer, W_score1)+B_score1)
            score1 = tf.layers.dropout(score1, rate=self.MLP_dropout, training=self.train)
            score = tf.matmul(score1, W_score2) + B_score2
    ...
    def compute_cost(self):
        ...
        with tf.variable_scope("loss"):
            ...
            self.loss = tf.reduce_mean(masked_loss) + self.l2*regularization + \
                self.Z_lambda*self.Z + self.sparse_lambda*self.sparse_reg
