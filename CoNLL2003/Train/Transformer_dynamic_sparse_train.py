...
from model.Transformer_dynamic_sparse_NER import Encoder
...
hyperparameters = config['CoNLL2003_Dynamic_Sparse_Hyperparameters']
...
model = Encoder(config_key='CoNLL2003_Dynamic_Sparse_Hyperparameters',
                text=tf_text,
                labels=tf_labels,
                dim=300,
                beta=tf_beta,
...
with tf.Session() as sess:  # Start Tensorflow Session
    ...
    if load.lower() == 'y':
        ...
        saver.restore(
            sess, '../Model_Backup/Transformer_Dynamic_Sparse/Transformer_Dynamic_Sparse.ckpt')
        sess.run(tf.global_variables())
        sess.run(tf.tables_initializer())

        with open('../Model_Backup/Transformer_Dynamic_Sparse/Transformer_Dynamic_Sparse.pkl', 'rb') as fp:
    ...
    for epoch in range(epochs_covered, epochs):
        ...
        for i in range(0, min(1000, len(train_batches_source))):
            ...
            cost, prediction,\
                sparsity, _ = sess.run([model.display_loss,
                                        model.predictions,
                                        model.sparse_reg,
                                        model.train_ops],
            ...
            iterations = min(1000, len(train_batches_source))
            T = (epochs//2+epochs//3)*iterations

            if epoch < (epochs//2+epochs//3):
                beta = beta*((500.0)**(1/T))
        ...
        for i in range(0, len(val_batches_source)):
            ...
            cost, prediction,\
                sparsity, acc = sess.run([model.display_loss,
                                          model.predictions,
                                          model.sparse_reg,
                                          model.accuracy],
        ...
        if flag == 1:
            ...
            saver.save(sess, '../Model_Backup/Transformer_Dynamic_Sparse/Transformer_Dynamic_Sparse.ckpt')
            ...
            with open('../Model_Backup/Transformer_Dynamic_Sparse/Transformer_Dynamic_Sparse.pkl', 'wb') as fp:
