...
from model.Transformer_sparse_NER import Encoder
...
hyperparameters = config['CoNLL2003_Sparse_Hyperparameters']
...
fc_dim = hyperparameters['fc_dim']

duty_cycles = np.zeros((layers*(3*dim+fc_dim)), np.float32)
...
tf_duty_cycles = tf.placeholder(tf.float32, [None])
...
model = Encoder(config_key='CoNLL2003_Sparse_Hyperparameters',
                text=tf_text,
                labels=tf_labels,
                duty_cycles=tf_duty_cycles,
                dim=300,
...
with tf.Session() as sess:  # Start Tensorflow Session
    ...
    if load.lower() == 'y':
        ...
        saver.restore(sess, '../Model_Backup/Transformer_sparse/Transformer_sparse.ckpt')
        sess.run(tf.global_variables())
        sess.run(tf.tables_initializer())

        with open('../Model_Backup/Transformer_sparse/Transformer_sparse.pkl', 'rb') as fp:
        ...
        duty_cycles = train_data['duty_cycles']
    ...
    for epoch in range(epochs_covered, epochs):
        ...
        for i in range(0, min(1000, len(train_batches_source))):
            ...
            cost, prediction,\
                duty_cycles, _ = sess.run([model.display_loss,
                                           model.predictions,
                                           model.new_duty_cycles,
                                           model.train_ops],
                                          feed_dict={tf_text: train_batches_source[j],
                                                     tf_labels: train_batches_labels[j],
                                                     tf_duty_cycles: duty_cycles,
        ...
        if flag == 1:
            ...
            saver.save(sess, '../Model_Backup/Transformer_sparse/Transformer_sparse.ckpt')
            ...
            with open('../Model_Backup/Transformer_sparse/Transformer_sparse.pkl', 'wb') as fp:
